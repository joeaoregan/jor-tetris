(() => {
  "use strict";
  var e,
    v = {},
    _ = {};
  function n(e) {
    var l = _[e];
    if (void 0 !== l) return l.exports;
    var r = (_[e] = { exports: {} });
    return v[e](r, r.exports, n), r.exports;
  }
  (n.m = v),
    (e = []),
    (n.O = (l, r, s, f) => {
      if (!r) {
        var u = 1 / 0;
        for (a = 0; a < e.length; a++) {
          for (var [r, s, f] = e[a], c = !0, o = 0; o < r.length; o++)
            (!1 & f || u >= f) && Object.keys(n.O).every((p) => n.O[p](r[o]))
              ? r.splice(o--, 1)
              : ((c = !1), f < u && (u = f));
          if (c) {
            e.splice(a--, 1);
            var t = s();
            void 0 !== t && (l = t);
          }
        }
        return l;
      }
      f = f || 0;
      for (var a = e.length; a > 0 && e[a - 1][2] > f; a--) e[a] = e[a - 1];
      e[a] = [r, s, f];
    }),
    (n.o = (e, l) => Object.prototype.hasOwnProperty.call(e, l)),
    (() => {
      var e = { 121: 0 };
      n.O.j = (s) => 0 === e[s];
      var l = (s, f) => {
          var o,
            t,
            [a, u, c] = f,
            i = 0;
          if (a.some((h) => 0 !== e[h])) {
            for (o in u) n.o(u, o) && (n.m[o] = u[o]);
            if (c) var d = c(n);
          }
          for (s && s(f); i < a.length; i++)
            n.o(e, (t = a[i])) && e[t] && e[t][0](), (e[t] = 0);
          return n.O(d);
        },
        r = (self.webpackChunkjor_tetris = self.webpackChunkjor_tetris || []);
      r.forEach(l.bind(null, 0)), (r.push = l.bind(null, r.push.bind(r)));
    })();
})();
